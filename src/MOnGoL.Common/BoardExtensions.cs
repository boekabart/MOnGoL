...
namespace MOnGoL.Common
{
    public static class BoardExtensions
    {
        ...
        public static PlacedToken? TokenAt(this Board aBoard, Coordinate coordinate) => aBoard.Tokens[coordinate.X + aBoard.Width * coordinate.Y];
        public static Board WithChanges(this Board aBoard, ChangeSet changes)
        {
            var newBoard = new Board(aBoard.Width, aBoard.Height, (PlacedToken[])aBoard.Tokens.Clone());
        ...
        public static IEnumerable<IEnumerable<(Coordinate Coordinate, PlacedToken? Token)>> GetColumns(this Board aBoard)
            => Enumerable.Range(0, aBoard.Width).Select(columnNumber => aBoard.GetColumn(columnNumber));
        public static IEnumerable<IEnumerable<(Coordinate Coordinate, PlacedToken? Token)>> GetRowsAndColumns(this Board aBoard)
            => aBoard.GetRows().Concat(aBoard.GetColumns());
        public static IEnumerable<(Coordinate Coordinate, PlacedToken? Token)> GetSlash(this Board aBoard, int startColumnNumber)
        ...
        public static IEnumerable<IEnumerable<(Coordinate Coordinate, PlacedToken? Token)>> GetSlashes(this Board aBoard)
